        ...
        if "cityscapes" in self.config.DATA.NAME:
            ...
            if "ood" in self.config.DATA.NAME:
                ...
                self.train_dataset = CarlaCOCOMix(
                    ...
                    coco_root=self.config.DATA.COCO_ROOT,
                    coco_proxy_size=self.config.DATA.COCO_PROXY_SIZE,
                    paste_mode=getattr(self.config.DATA, "COCO_PASTE_MODE", "perspective"),
                    horizon_y_ratio=getattr(self.config.DATA, "COCO_HORIZON_Y_RATIO", 0.45),
                    min_scale=getattr(self.config.DATA, "COCO_MIN_SCALE", 0.2),
                    max_scale=getattr(self.config.DATA, "COCO_MAX_SCALE", 1.35),
                    scale_jitter=getattr(self.config.DATA, "COCO_SCALE_JITTER", 0.1),
                    allowed_host_labels=getattr(self.config.DATA, "COCO_ALLOWED_HOST_LABELS", None),
                    coco_fog_p=getattr(self.config.DATA, "COCO_OBJECT_FOG_P", 0.0),
                    coco_fog_strength=getattr(self.config.DATA, "COCO_OBJECT_FOG_STRENGTH", 0.35),
                    hparams=self.config.DATA,
                    transform=transformations.cityscapes_train,
                    split="train",
                )
